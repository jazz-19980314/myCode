	...
	<view class="bigContent">
		...
		<view class="main_container" v-for="(item,index) in datalist" :key="index">
			...
			<view class="frominfo">
				<view class="priceinfo">
					...
					<input type="digit" placeholder="请输入您的销售价格" name="input" value="item.salesPrice" v-model="salesPrice[index]" @input="inputPrice(index)" style="color: #333;"></input>
				</view>
				<!-- 订单备注 -->
				<view class="usermessage">
					<text class="tag-blue">商品备注</text>
					<input type="text" placeholder="请输入您的备注信息" name="input" value="item.remark" v-model="remark[index]" @input="inputMessage(index)"></input>
				</view>
			</view>
		</view>
		<!-- 订单留言 -->
		...
		<view class="bottom_offer">
			<view class="bottom_left">
				报价金额<text style="color:  rgba(2, 160, 254, 1);font-size: 14px;margin-left: 40rpx;">￥</text><text style="font-size: 20px;color: rgba(2, 160, 254, 1);font-weight: bold;">{{allprice}}</text>
			</view>
			...
			enterInfoSelect(){
				this.$http.postquestionInfo(JSON.stringify(this.datalist)).then(res=>{
					if(res.data.ok){
						...
						// uni.navigateTo({
						// 	url:'../quotationcenter/quotationcenter?type=1'
						// })
	...
	.bigContent {
		...
		.basicInfo {
			...
			.firstline {
				...
				color: rgba(51, 51, 51, 1);
				line-height: 22px;
				padding-left: 30rpx 40rpx 40rpx 40rpx;
				position: relative;
				padding-left: 40rpx;
