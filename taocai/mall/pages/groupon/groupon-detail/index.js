...
Page({
  ...
  onShareAppMessage: function () {
    ...
    let path = ''
    path = 'pages/groupon/groupon-detail/index?id=' + grouponInfo.id
    return {
      title: title,
      path: path,
      imageUrl: imageUrl,
  ...
  //查询拼团信息
  grouponInfoGet() {
    app.api.grouponInfoGet(this.data.id)
      .then(res => {
  ...
  grouponUserPageGrouponing(){
    app.api.grouponUserPageGrouponing(Object.assign(
      {
        grouponId: this.data.id,
        isLeader: '1',
        status: '0'
      },
      this.data.page,
      ...
      .then(res => {
        ...
        this.setData({
          ...
          grouponUserTotal: res.data.total
        })
  ...
  onCreatePoster() {
    app.api.qrCodeUnlimited({
      ...
      scene: this.data.grouponInfo.id
    })
      .then(res => {
        let base64 = res.data
