...
<template>
  <div class="execution">
    <basic-container>
      ...
      >
        <template slot="menuLeft">
          <el-button
            ...
            v-if="permissions['mall:goodsspu:edit']"
          >批量上架</el-button>
          <el-button
            type="warning"
            @click="batchShelf('0')"
        ...
        <template slot="price" slot-scope="scope">
          <div style="color: red">
            ￥{{scope.row.priceDown}}{{scope.row.priceUp == scope.row.priceDown ? '' : '~￥'+
            scope.row.priceUp}}
          </div>
        </template>
      ...
      <el-dialog title="商品评价" :visible.sync="dialogAppraises" width="40%">
...
export default {
  ...
  created() {},
  mounted: function() {
    if (this.permissionList.categorymore) {
  ...
  computed: {
    ...
    permissionList() {
      return {
        categorymore: this.permissions["mall:goodscategorymore:index"]
          ? true
          : false,
  ...
  methods: {
    ...
    //分类列表
    getCategoryMore() {
      getCategoryMoreList().then(res => {
        this.getCategoryMoreList = res.data;
      });
    },
    ...
    beforeOpen(done, type) {
      if (!this.permissionList.categorymore) {
        this.tableOption.group[0].column[1].display = false;
        this.tableOption.group[0].column[1].dicUrl = '';
      }
      if (type == "add") {
        this.goodsSpuSpec = [];
        this.goodsSku = [];
    ...
    getPage(page, params) {
      ...
      if (this.paramsSearch.categoryThirdId) {
        this.paramsSearch.categoryThird = this.paramsSearch.categoryThirdId[1];
      }
      getPage(
        Object.assign(
          {
        ...
        .then(response => {
          ...
          let getCategoryMoreLists = this.getCategoryMoreList;
          tableData.forEach(function(item, index) {
            ...
            let categoryThirdId = [];
            if (getCategoryMoreLists) {
              if (item.categoryThird) {
                getCategoryMoreLists.forEach(data => {
                  if (data.id == item.categoryThird) {
                    categoryThirdId.push(data.parentId);
                    categoryThirdId.push(data.id);
                    item.categoryThirdId = categoryThirdId;
                  }
                });
              }
            }
    ...
    handleUpdate: function(row, index, done, loading) {
      ...
      row.categoryThird = row.categoryThirdId[1];
