		...
		<avue-crud :option="option" :table-loading="loading" :data="data" :page.sync="page" :permission="permissionList"
		 ...
		 @size-change="sizeChange" @refresh-change="refreshChange" @on-load="onLoad">
			<template slot="menuLeft">
				<el-button type="danger" size="small" icon="el-icon-delete" plain v-if="permission.claim_delete" @click="handleDelete">删
					除
				</el-button>
			</template>
			<template slot="menu">
				<el-button type="text" size="small" icon="el-icon-download" circle="" @click="downloadFile">
					附件
				</el-button>
			</template>
			<template slot="criteriaListForm">
				<div>
					<el-form ref="forms" :model="forms">
						<el-form-item v-for="(data, index) in forms" :key="index" v-model="form[index]" class="el_from_item">
                  ...
                  ">
                    ...
                    index + 1
									...
									<div style="display: flex">
										<el-button type="success" circle="" icon="el-icon-circle-plus-outline" @click="handelAdd(index)"></el-button>
										<el-button @click="handelDelete(index)" type="danger" icon="el-icon-delete" circle></el-button>
									</div>
								</div>
								<div class="bigContent clearfix" style="margin: 5px; width: 100%">
									<!-- 初始条件 -->
									<div style="display: flex; margin: 5px" v-for="(item, indexs) in forms[index].inputList" :key="item">
										<div style="width: 200px">
											<el-select @focus="setDataIndex(index)" v-model="forms[index].targetValue[indexs]" filterable placeholder="请选择指标"
											 @change="
                          (data) => {
                        ...
                        ">
										...
										<div style="width: 60px; margin-right: 5px" v-if="forms[index].targetType[indexs] == '1'">
											<el-autocomplete @focus="setDataIndex(index)" class="inline-input" v-model="forms[index].operatorValue[indexs]"
											 :fetch-suggestions="querySearch" placeholder="" @select="
                          (data) => {
                            operatorChange_1(data, index);
                          }
                        "></el-autocomplete>
										</div>
										<div style="width: 100px; margin: 0px 5px 0 0" v-show="
                        ...
                        forms[index].operatorValue[indexs] == '='
                      ">
											<el-input @focus="setDataIndex(index)" v-model="forms[index].choicesNumber[indexs]">
												<!-- <template slot="append">万元</template> -->
											</el-input>
										</div>

										<div style="width: 100px; margin: 0px 5px 0 0" v-if="forms[index].targetType[indexs] == '2'">
											<el-input @focus="setDataIndex(index)" v-model="forms[index].choicesString[indexs]"></el-input>
										</div>
										<!-- 单选条件 -->
										<div style="margin: 5px 5px 0 0" v-if="forms[index].targetType[indexs] == '3'">
											<el-radio-group v-model="forms[index].radioValue[indexs]">
												<el-radio :label="item.label" v-for="item in forms[index].radioDic[indexs]" :key="item.value">{{ item.label }}</el-radio>
											</el-radio-group>
										</div>
										<!-- 多选条件 -->
										<div style="margin: 5px 5px 0 0" v-if="forms[index].targetType[indexs] == '4'">
											<el-checkbox-group v-model="forms[index].checkboxValue[indexs]">
												<el-checkbox :label="item.label" v-for="item in forms[index].checkboxDic[indexs]" :key="item.value">{{ item.label }}</el-checkbox>
											</el-checkbox-group>
										</div>
									</div>
									<!-- 并且条件 -->
									<!-- <br v-if="isHuo"> -->
									<div class="bing" style="display: flex; margin: 5px" v-for="(item, indexs) in forms[index].inputChildList"
									 :key="indexs" :class="item == '|' ? 'nextLine' : ''">
										<!-- <div style="width" v-if="item == '|'"></div> -->
										<div style="margin: 0px 10px 0 0; color: red" v-if="
                      ...
                      ">
										...
										<div style="margin: 0px 10px 0 0; color: green" v-if="forms[index].inputChildList.length && item == '|'">
											或者
										</div>
										<div style="width: 200px">
											<el-select v-model="
                        ...
                        "
											 filterable placeholder="请选择指标" @focus="setDataIndex(index)" @change="
                          (data) => {
                            targetChange(data, index);
                          }
                        ">
										...
										<div style="width: 10px"></div>
										<div style="width: 60px; margin-right: 5px" v-if="
                        forms[index].targetType[
                          indexs + forms[index].inputList.length
                        ] == '1'
                      ">
											<el-autocomplete @focus="setDataIndex(index)" class="inline-input" v-model="
                      ...
                      ">
											<el-input @focus="setDataIndex(index)" v-model="
                        ...
                        ">
										...
										<div style="width: 100px; margin: 0px 5px 0 0" v-if="
                        forms[index].targetType[
                          indexs + forms[index].inputList.length
                        ] == '2'
                      ">
                        ...
                        "></el-input>
										...
										<!-- 单选条件 -->
										<div style="margin: 5px 5px 0 0" v-if="
                        forms[index].targetType[
                          indexs + forms[index].inputList.length
                        ] == '3'
                      ">
                        ...
                        ">
												<el-radio :label="item.label" v-for="item in forms[index].radioDic[indexs + forms[index].inputList.length]"
                      ...
                      ">
                        ...
                        ">
												<el-checkbox :label="item.label" v-for="item in forms[index].checkboxDic[indexs + forms[index].inputList.length]"
								...
								<div style="margin-top: 5px">
									<!-- 并且按钮 -->
									<el-button type="text" icon="el-icon-circle-plus-outline" @click="handelAddChild(index)">并且</el-button>
									<!-- 或者按钮 -->
									<el-button type="text" icon="el-icon-circle-plus-outline" @click="handelAddHuo(index)">或者</el-button>
								</div>
							</div>
						</el-form-item>
					</el-form>
			...
			<template slot="virtualNum" slot-scope="{ row }">
				<span style="color: #409eff; cursor: pointer" @click="openVirtualDialog(row)">{{ row.virtualNum }}</span>
			</template>
		</avue-crud>
		<!-- 虚拟符合数 -->
			...
			targetChange(data, index) {
				...
				if (target.length) {
					if (target[0].type == "1") {
						...
						this.forms[index].checkboxDic.push([]);
						this.forms[index].radioDic.push([]);
					} else {
						this.isNumber = false;
					}
					if (target[0].type == "2") {
